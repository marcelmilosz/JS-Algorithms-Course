...
// === 
// IT TOOK YOU 3 HOURS HOLLY F, but was easy tbh
// ===
...
function minSubArrayLen(arr, min) {
    ...
    if (sum < min) return 0 // Checking if sum is smaller then required min value

    sum = 0;
    let left = 0; // starting left pointer
    let right = 0; // starting right pointer
    let actuallDigits = 0;
    let minDigits = arr.length;
    ...
    while (true) {
        ...
        if ((sum - arr[left]) >= min) {
            ...
            if (actuallDigits < minDigits) minDigits = actuallDigits;
        }
        else {
            if (right == arr.length) return minDigits
